...
const CheckboxGroup = ({ onChange }) => {
  const checkTypes = [
    { name: '시험년도', count: 5, startIndex: 2022, increment: false, isString: true },
  ...
  return (
      <div className="CheckboxGroupContainer">
        {checkTypes.map((checkboxType) => (
            <div className="checkbox1" key={checkboxType.name}>
              <h2>{checkboxType.name}</h2>
              {Array.from({ length: checkboxType.count }, (_, index) => (
                  <div className="checkbox2" key={`${checkboxType.name}-${index}`} style={{ display: 'inline-block', margin: '10px' }}>
                    <input
                        type="checkbox"
